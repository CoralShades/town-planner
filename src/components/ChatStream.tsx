...
// Function to fetch report content from database
const fetchReportContent = async (reportId: string): Promise<{ content: string; metadata: Record<string, unknown> } | null> => {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('file_path, file_format, generated_content, title, topic, address, metadata')
      .eq('id', reportId)
      .single();
    ...
    // Use generated_content if available, otherwise we could fetch from file_path
          ...
          if (parsed && typeof parsed === 'object' && 
            ...
            if (reportContent && reportContent.trim()) {
              ...
              if (reportId) {
                ...
                // Final fallback to ReportDisplay
                console.log('⚠️ No direct content and no report ID, falling back to ReportDisplay');
                const reportData = {
                  action: parsed.action,
                  topic: parsed.topic || parsed.title || 'Planning Report',
                  address: parsed.address || parsed.location,
      ...
      if ((obj.action === 'report' || obj.action === 'change_report' || obj.action === 'generate_report') && 
        ...
        if (reportContent && reportContent.trim()) {
          ...
          if (reportId) {
            ...
            // Final fallback to ReportDisplay
            console.log('⚠️ No direct object content and no report ID, falling back to ReportDisplay');
            const reportData = {
              action: obj.action as string,
              topic: (obj.topic || obj.title || 'Planning Report') as string,
              address: (obj.address || obj.location) as string,
